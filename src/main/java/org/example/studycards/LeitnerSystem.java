...
public class LeitnerSystem extends StudyMethod {
    ...
    public String getRandomCard(List<Box> otherBoxes) {
        ...
        Box allCardsBox = collectAllCardsFromBoxes(otherBoxes);
        Integer randomCardId = allCardsBox.getRandomCard();
        return formatRandomCardOutput(randomCardId);
    }

    private Box collectAllCardsFromBoxes(List<Box> otherBoxes) {
    ...
    private String formatRandomCardOutput(Integer cardId) {
        if (cardId == null) {
        ...
        StringBuilder response = new StringBuilder();
        response.append("[").append(cardId).append("] ");
