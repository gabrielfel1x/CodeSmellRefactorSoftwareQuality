...
public class StudyTaskManager {
    ...
    // New clean method using WeekSetup object
    public void setUpWeek(WeekSetup weekSetup) {
        this.weekResponsibilities = weekSetup.toList();
    }

    // Method required by your tests — handles List<String> input
    public void handleSetUpWeek(List<String> stringProperties) {
        if (stringProperties == null || stringProperties.size() < 11) {
            throw new IllegalArgumentException("Expected list with at least 11 elements");
        }
